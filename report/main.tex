...
\chapter{Conclusion}
This thesis delivered an MVP for a CTF competition platform built mostly with open-source software. The platform enables users to launch, verify, and terminate challenge environments on demand via a RESTful API integrated with CTFd, supporting both VMs and containers. Secure access is achieved through a bastion host safeguarded by a PKI, complemented by Keycloak for single sign-on and role-based access control. Observability is ensured using Prometheus, Grafana, Loki, and Promtail, while dynamic storage provisioning guarantees data persistence where required. Comprehensive documentation is provided via a Jekyll-powered website, and deployment on both UCloud and GCP underscores the platform's portability, adaptability, and cloud-agnostic design.
...
Platform usage is documented through a Jekyll-based static site that guides users in API access, challenge development, SSH-based interaction, and certificate trust configuration. Fault tolerance was validated through chaos testing, and dynamic storage provisioning using NFS, MinIO, or GCP persistent disks ensures consistent data persistence across deployments. In the event of server node restarts or shutdowns, the system remains recoverable, though PKI-related challenges may sometimes require minor manual intervention.

IAM is enforced via Keycloak, with OIDC-based authentication enabled across multiple services. A custom plugin extends this to CTFd. The PKI is tightly integrated with the platform's security model, enabling SSH access via certificates and securing infrastructure-level HTTPS traffic. Though the Deployer Backend relies on access tokens issued by Keycloak, it could be made more user-friendly by integrating token retrieval via the Resource Owner Password Credentials grant.

The Deployer Backend now supports automated solution validation scripts and has been enhanced for compatibility with both VMs and containers through an Unleash feature flag. While adding server-side HTTPS support is a potential future enhancement, the backend is currently configured to trust outbound traffic secured by a designated PKI certificate, ensuring that connections to services using valid PKI-issued certificates are trusted. Comprehensive Swagger documentation is provided for all relevant endpoints.
...
Planned future improvements include persisting logs in Loki for better post-execution visibility, extending logging to containerized challenges, and integrating Keycloak authentication with Unleash. The Deployer Backend, while functional, may benefit from a refactor for greater extensibility. Replacing Terraform with Pulumi and adopting NixOS for VM image management would streamline infrastructure-as-code and further unify the toolchain.
