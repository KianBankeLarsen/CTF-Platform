...
\usepackage[block=ragged, style=ieee, backend=biber]{biblatex}
...
\begin{abstract}
\paragraph{English}
This thesis presents a minimal viable CTF competition platform built with open-source components to support practical cybersecurity training. Motivated by limitations in existing solutions, such as complex deployment processes and lack of automated challenge testing, the platform uses a microservice-based architecture to provision challenge environments on demand. It enables users to launch, verify, and terminate CTF challenges through a RESTful API integrated with a CTFd frontend, supporting both Docker container and VM deployment modes. Secure SSH access to each challenge is enforced through a bastion host protected by PKI, and user authentication and role-based access control are managed via Keycloak. System observability is provided through Prometheus, Grafana, and Loki, while dynamic storage provisioning ensures persistent user data. The platform is documented via a static website and was successfully deployed on both the university's UCloud infrastructure and GCP, demonstrating its portability and flexible infrastructure support. The resulting MVP simplifies CTF competition setup and offers a reusable foundation for future cybersecurity training environments.

\paragraph{Danish}
Dette speciale præsenterer en minimal levedygtig CTF-konkurrence-platform opbygget med open source-komponenter til at understøtte praktisk cybersikkerhedstræning. Platformen er motiveret af begrænsninger i eksisterende løsninger, såsom komplekse udrulningsprocesser og mangel på automatiseret test af CTF'er, og benytter en mikroservice-baseret arkitektur til at klargøre CTF-miljøer efter behov. Brugere kan starte, verificere og afslutte CTF'er via et RESTful API integreret med en CTFd-frontend, og platformen understøtter både Docker-containere og VMs. Sikker SSH-adgang til hver CTF håndhæves gennem en bastion-host beskyttet af PKI, og brugerautentifikation samt rollebaseret adgangskontrol administreres via Keycloak. Systemovervågning leveres med Prometheus, Grafana og Loki, mens dynamisk lagerallokering sikrer vedvarende brugerdata. Platformen er dokumenteret via en statisk hjemmeside og blev med succes udrullet på både SDUs UCloud-infrastruktur og GCP, hvilket demonstrerer dens portabilitet og fleksible understøttelse af forskellige infrastrukturer. Den resulterende MVP forenkler opsætningen af CTF-konkurrencer og udgør et genanvendeligt fundament for fremtidige cybersikkerhedstræningsmiljøer.
\end{abstract}
...
\begin{itemize}
    \item Jeopardy revolves around a set of challenges provided by the competition organizers. They are mainly focused on exploiting a system in order to reveal a small piece of text or ``flag''. 
...
This thesis will not go into detail regarding the CTF challenges themselves, but rather focus on the infrastructure and architecture of a platform hosting such challenges. We will provide some very simplistic Jeopardy challenges to demonstrate the platform's capabilities.
...
Dynamic storage provisioning is a feature of Kubernetes that facilitates the automatic creation and management of storage resources. This capability is highly useful for applications that need persistent storage, such as databases or file systems. While dynamic provisioning is not strictly necessary, it can make processes much more straightforward. Helm Charts, a Kubernetes deployment tool that bundles and templates resources, typically include Persistent Volume Claims (PVCs), but not Persistent Volumes (PVs), which can add an extra step when manually linking PVs with PVCs.
...
Every certificate contains information about its issuer, such that the trust chain can be verified. The complete trust chain is presented when inspecting the trust chain in the browser, except for the root certificate. It does not have to be bundled because the browser already has a list of default trusted root certificates. Every certificate in the chain serves a specific purpose.
...
\subsection{Service Mesh}\label{sec:service_mesh}
...
For log aggregation, Grafana Loki was chosen because it is part of the same \textit{Grafana} ecosystem and provides an open-source set of components that can be composed into a fully featured logging stack \parencite{grafana_loki}. Loki is licensed under AGPLv3 \cite{grafana_loki_license}.
...
We will not focus on selecting a Feature Flag System. Unleash has provided an objective list of OSS tools that address this need \cite{UnleashFeatureFlags}. This is notable, given that Unleash specializes in Feature Flags. It is licensed under the Apache-2.0 license, has 12k stars and 759 forks on GitHub \cite{UnleashRepo}, and has been ranked as the top Feature Management Software on G2 \cite{UnleashFeatureFlags}. Based on this, we will examine the main features of Unleash rather than other tools. While Unleash offers a more feature-rich enterprise version, the OSS features are sufficient for this project.

\subsection{The Anatomy of Unleash}
Unleash manages feature flags using a client-server architecture. The server controls the feature flags, while the client retrieves and evaluates them via a REST API. The exported endpoints are typically wrapped by a client library, such as the unleash-client-go library for Go, developed by Unleash \cite{unleash_client_go}.

Feature flags are organized within projects, which can be used to group related feature flags, teams, or modules. A project can also be assigned per tenant to simulate multi-tenancy. The OSS version supports one project, the Pro version allows up to three, and the Enterprise version supports up to 500 \cite{unleash_resource_limits}.

Projects contain environments. The OSS version permits two environments \cite{unleash_resource_limits}, which is sufficient for a production environment and an optional development environment if the environments differ. A feature flag is then assigned to an environment. 

\subsection{Activation Strategies}
An activation strategy defines which users receive a feature. It is assigned to a feature flag within an environment. A feature flag can have multiple activation strategies, each evaluated independently. A collection of such reusable activation strategies is called a segment \cite{unleash_activation_strategies}.

The default activation strategy is a 100\% gradual rollout, adjustable by setting rollout percentage, targeting, and variants. The rollout percentage determines how many users receive the feature. Targeting specifies which users should be affected, using constraints defined by a context field, an operator, and a value. This allows filtering based on any context. Variants extend a feature flag's payload with additional data to create different versions \cite{unleash_activation_strategies}.
...
GitHub Pages is a free service to host a static website. It is a great option for hosting documentation, as it is easy to set up and integrates well with GitHub repositories -- it is not even necessary to create a pipeline to deploy the website, as everything is handled by GitHub after enabling the feature. GitHub Pages allows one site per repository \cite{GitHubPagesLimits}.
...
VMs are software packages that emulate low-level hardware components such as CPU, disk, and networking devices. They may also include a software stack to operate on the emulated hardware, creating a fully functional computational system snapshot \cite{atlassian_containers_vs_vms}.

VMs provide full isolation security, running as standalone systems. This prevents exploits from spreading between VMs on a shared host, ensuring that a compromised machine remains contained. They also support interactive development, allowing manual installation of software and the creation of snapshots to preserve specific configurations. These snapshots can be used to restore a VM to a previous state or deploy additional instances with identical setups \cite{atlassian_containers_vs_vms}.

Despite these advantages, VMs have limitations. Iteration speed is slower since each snapshot represents an entire system, requiring significant time to regenerate and validate modifications. Additionally, storage size cost can be high, as VMs often occupy several gigabytes of disk space, potentially leading to storage constraints on the host system \cite{atlassian_containers_vs_vms}.

\subsection{KubeVirt}
KubeVirt is an open-source virtualization API extension for Kubernetes, licensed under the Apache License 2.0 \cite{kubevirt_license} and maintained as a Cloud Native Computing Foundation (CNCF) project \parencite{kubevirt_cncf}. It enables the management of VMs alongside containers by introducing custom resource definitions (CRDs), which extend Kubernetes to support VM workloads using its native tools. This allows users to run both containerized and virtualized applications within the same cluster, creating a unified infrastructure for diverse workloads.
...
It can build Copy-On-Write version 2 (QCOW2) disk images for VMs \cite{nixos_vm_configuration}. Using a Nix expression, users generate a QCOW2 image with a predefined environment, ensuring consistency and eliminating manual setup \cite{nix_language_tutorial}.

NixOS configurations can be tested in VMs without installation on physical hardware. Unlike interactive configuration, which lacks documentation, NixOS provides a structured approach. Interactive setups make modifications difficult and do not track changes, leading to uncertainty about the image's contents.
...
\begin{itemize}
    \item On-demand challenge deployment in isolated VMs, mitigating container escape risks.
    \item Use of Docker Compose files for both local development and remote deployment, easing the development workflow.
    \item Automated and manual testing of challenges via an API, without requiring administrative access (not achieved as part of his thesis).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Reflector:} A Kubernetes utility that automatically mirrors secrets across namespaces.
    ...
    \begin{itemize}
        ...
        \item \textbf{KubeVirt:} Extends Kubernetes by adding support for running VM workloads alongside container workloads.
    \end{itemize}
\end{itemize}
...
In case the passphrase has been compromised, it can be updated using the command:
...
\end{displayquote}
    \hspace*\fill{\small--- Smallstep, Proxying step-ca traffic \cite{smallstep_step_ca}}
...
\subsection{Services}\label{sec:services_tls}
...
The \texttt{verify} endpoint is responsible for deleting the corresponding namespace when a flag is submitted. This is necessary because the \texttt{solution} being executed cannot simply complete -- it will automatically restart. In a \texttt{Deployment}, the only supported restart policy is \texttt{Always}. The correct approach would be to use a Kubernetes \texttt{Job}, as this resource type allows completion. However, a \texttt{Job} can only be used in container mode, as running a VM as a \texttt{Job} is not possible.
...
Kubernetes resources for the Traefik controller were removed, and \texttt{Ingress} resources were patched to correctly use PKI-issued certificates. The server has also been configured to trust the PKI root certificate to allow secure HTTPS requests. 
...
\section{Docker Registry}
A Docker registry serves as more than just a storage solution for container images -- it also enables caching mechanisms and helps manage rate limits efficiently. This section explores how the already implemented registry can be leveraged more effectively to maximize its potential.
...
Some services, such as Keycloak, require TLS configuration in production mode \cite{bitnami-keycloak-values}, making a PKI valuable for certificate provisioning. However, as discussed in Section \ref{sec:services_tls}, configuring TLS across all services is not always straightforward, and some may lack support for it entirely. In such cases, a service mesh like Istio (Section \ref{sec:service_mesh}) provides a viable solution by handling TLS termination and certificate management, enabling secure communication without requiring individual services to implement TLS. This approach simplifies the methodology and enhances security by ensuring encrypted communication. Future work could experiment with the implementation of a service mesh.
...
As outlined in an Oracle blog by Christian Horn, hardware virtualization is a powerful technology that can significantly enhance performance and reduce the power consumption of VMs compared to hardware emulation. Performance improvements vary based on architecture, but an approximate fourfold speed increase is generally expected. However, no virtualization method can fully match the efficiency of running directly on bare metal \parencite{horn2025virtualization}.
...
\includepdf[pages={-},angle=0]{./appendix/hardware_virtualization_mail.pdf}
\end{document}