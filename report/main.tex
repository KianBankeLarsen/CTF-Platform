...
The backend currently requires users to send an access token in the request header for authentication. A more user-friendly approach would have been to support basic authentication with a username and password, allowing the API to request a token from Keycloak using the Resource Owner Password Credentials grant. This change would also have allowed the Keycloak client to be confidential, enabling internal authentication for specific endpoints. Implementing internal authentication now would require an additional client. The solution verification endpoint, in particular, would benefit from internal authentication, as it does not need direct exposure to users; only solution VMs should submit a flag for a solution. While users could still misuse this functionality, requiring all verified challenges to originate from a solution VM would provide a safeguard.

\subsection{Missing Features}
Although readiness probes have been implemented, the backend does not use them to verify whether a challenge is operational. Instead, it immediately returns \texttt{started: true} once the resources are created. This is technically accurate, as the instance is indeed started; however, the \texttt{status} and \texttt{start} endpoints would benefit from an additional field indicating whether the VM or container is ready to receive traffic. This would prevent users from repeatedly attempting to access the challenge to check its availability.
